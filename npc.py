...
class NPC(AnimatedSprite):
    def __init__(self, game, path='resources/sprites/npc/soldier/0.png', pos=(10.5, 5.5),
                 scale=0, shift=0, animation_time=180):
    ...
    def movement(self):
        ...
        if next_pos not in self.game.object_handler.npc_positions:
            angle = math.atan2(next_y + 0.5 - self.y, next_x +
                               0.5 - self.x)  # NPC의 각도와 타일의 중심
    ...
    def animate_death(self):
        if not self.alive:
            # 전역 변수의 40밀리초를 통해 npc 죽는 반응속도가 더 빨라짐
    ...
    def check_hit_in_npc(self):
        # 벽 뒤에서 총격을 확인하면 npc적중 방법 시선 검사(벽 뒤에서 쏘면 적 안맞게)
    ...
    def ray_cast_player_npc(self):
        ...
        # 세로
        x_vert, dx = (x_map + 1, 1) if cos_a > 0 else (x_map -
                                                       1e-6, -1)  # 방향 벡터
...
class CacoDemonNPC(NPC):
    def __init__(self, game, path='resources/sprites/npc/caco_demon/0.png', pos=(10.5, 6.5),
                 scale=0, shift=0, animation_time=250):
        super().__init__(game, path, pos, scale, shift, animation_time)
